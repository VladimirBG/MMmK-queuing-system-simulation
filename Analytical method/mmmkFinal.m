...
DialogM = inputdlg({'Средняя интенсивность потока поступления заявок (Lambda)' 'Средняя интенсивность потока обслуживания заявок (Mu)' 'Максимальная длина очереди (m)' 'Допустимое число требований в системе (K)' 'Время работы системы'}, 'Введите начальные параметры системы');
...
%% P0 - Начальные условия
P0 = [1;zeros(length(Pij)-1,1)];
T = [0,str2num(DialogM{5})];
[t,P] = ode23(@cmo, T, P0);
...
fprintf('Поступило заявок: %f\n', L);
fprintf('Среднее количество отклоненных заявок: %f\n', DialogM{5}*Pnot);
